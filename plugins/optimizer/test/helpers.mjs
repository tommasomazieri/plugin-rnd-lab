// Fixture builders for the optimizer suite. These make REAL git repos and REAL directory
...
export function tmpDir(label = 'optimizer-test-') {
...
export function makeRepo(files = { 'README.md': 'v1\n' }) {
  const repo = tmpDir('optimizer-repo-');
  git(repo, ['init', '-q', '-b', 'main']);
  git(repo, ['config', 'user.email', 'test@example.invalid']);
  git(repo, ['config', 'user.name', 'optimizer test']);
...
export function makeEnvPair(envJson, { seed = { 'seed.txt': 'seed\n' } } = {}) {
  const base = tmpDir('optimizer-env-');
