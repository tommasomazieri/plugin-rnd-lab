#!/usr/bin/env node
/**
 * session-context.mjs — SessionStart hook, fires in EVERY session optimizer is enabled in
 ...
 * /optimizer:init at least once); if absent, exits silently. If present, injects the current
...
function main() {
  ...
  if (!abBenchDir) {
    process.exit(0); // no .ab-bench/ anywhere above cwd — not an optimizer-tracked repo, stay silent
  }
  ...
  const lines = [
    'optimizer: this repo has an active experiment tracked in .ab-bench/ — you are the MAIN session (never an arm).',
    `  plugin repo: ${abBenchDir.replace(/\.ab-bench$/, '').replace(/[\\/]$/, '')}`,
    `  current mandate: ${state.current_mandate}${mandateExists ? '' : ' (mandate.md MISSING — run /optimizer:understand)'}`,
    `  current env: ${state.current_env}${envExists ? '' : ' (env.json MISSING — run /optimizer:init)'}`,
    `  testenv (runs/.dod/seed/ledger/baselines): ${paths.testenvRoot}`,
    '  optimizer skills (init/understand/plan/fire/analyze/status) resolve these automatically — no experiment name needed unless you want to target a different env.',
  ];
...
try {
  ...
  console.error(`[optimizer] session-context hook error (non-fatal): ${e.message}`);
  process.exit(0);
}
