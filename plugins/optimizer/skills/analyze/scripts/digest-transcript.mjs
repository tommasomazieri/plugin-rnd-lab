...
export function digestFile(filePath, arm) {
  ...
  if (d.checker_prompt_line) {
    d.warnings.push(
      ...
      'optimizer\'s DOD_LITE_CHECKER guard failed to skip it, so it was linked in manifest.json and ' +
...
function main() {
  ...
  if (!fs.existsSync(file)) {
    console.error(`[optimizer] transcript not found: ${file}`);
    process.exit(1);
  }
  ...
  if (out) {
    ...
    console.log(`[optimizer] digest written: ${out} (${(md.length / 1024).toFixed(1)} KB from ${d.jsonl_lines} JSONL lines)`);
    for (const w of d.warnings) console.log(`[optimizer] WARNING (${arm}): ${w}`);
  } else {
    console.log(md);
  }
}
