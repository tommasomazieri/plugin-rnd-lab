---
description: >-
  Teach the user how optimizer works end-to-end: setup, planning an
...
`plugins/optimizer/skills/` rather than guessing. `plugins/dod-lite/` has no
skills of its own to read — it's a hooks-only engine, see the dod-lite
section below.

## The one-paragraph mental model

You built (or are evaluating) a Claude Code plugin. optimizer proves whether
...
optimizer's own skills inside either arm. That main session runs FROM the
plugin-under-test's own repo (cd into it, start Claude Code there) — optimizer
...
One-time (or whenever you want to relocate). optimizer needs exactly one
...
asked for automatically the first time you enable the plugin; `/optimizer:setup`
...
plugin you ever test. Every other optimizer skill refuses to run until this
is set.
...
chains into `/optimizer:understand` to write `mandate.md`. Interviews you for:
...
branches: continue to `/optimizer:plan` for the next run, start a **new env**
...
`/optimizer:understand` for a full re-interview). This is where the central
...
drafting task.md or picking DoD checks. `/optimizer:init` now MANDATORILY invokes this right
...
config delta. Re-invoke it standalone (`/optimizer:understand`, run from the plugin repo) whenever
the plugin's scope evolves — it'll offer to refresh the CURRENT mandate in place, or point you at
`/optimizer:init`'s "new mandate" branch if the change is big enough to warrant a whole new version
instead. Full re-interview either way, never a partial patch. `/optimizer:plan` refuses to draft a
...
optimizer doesn't automate either arm's conversation. Practical rules while
both are running:
...
test in ITS OWN repo, in a separate session — optimizer never edits the
plugin under test. Then `/optimizer:plan` again for the next run.
...
## dod-lite — why optimizer leans on it
...
enforced every turn via its Stop hook. It's what lets `/optimizer:plan`
...
separately installed. It's mandatory on every run: `/optimizer:fire` injects
it into both arms automatically (`--plugin-dir`, read live off this repo's
disk — nothing to install or update separately), and `/optimizer:plan` is
...
optimizer and dod-lite together need nothing beyond Node builtins and Claude
Code itself — no npm packages, no other plugins required. The one optional
exception: `/optimizer:analyze`'s comparator agent can use the third-party
...
- **Never edit `env.json` after a run has fired against that experiment.**
  ...
  exception — refresh it anytime via `/optimizer:understand`, no version bump.
- **`task.md` must stay plugin-blind.** Any mention of the plugin under
  test, its tools, or a hinted workflow contaminates the run.
- **`/optimizer:plan` won't draft anything without `mandate.md`.** It's the
...
- **DoD tracking is mandatory, not something to declare in `env.json`** —
  ...
  per-run choice is whether `/optimizer:plan` writes any checks at all.
...
next command given where they are: nothing set up yet → `/optimizer:setup`;
set up but no experiment in this repo yet → cd into the plugin repo, run
`/optimizer:init`; experiment exists, no plan → `/optimizer:plan`; planned but
not fired → `/optimizer:fire`; fired →
go work both arms; both arms done → `/optimizer:analyze`.
