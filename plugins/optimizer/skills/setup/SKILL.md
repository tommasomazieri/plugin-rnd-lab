...
description: >-
  ...
  repo). User-invoke only. Run this once after installing optimizer, before
  /optimizer:init. Safe to re-run any time to change the location.
...
optimizer needs ONE folder outside any project repo to hold what every experiment run actually
materializes on disk (`<experiments_root>/<plugin-folder-name>/mandate-N/env-M/…` — seed files,
`.dod/`, run history, transcripts; auto-derived from the plugin repo you run `/optimizer:init`
...
`~/claude-optimizer-experiments`) and ask the user to confirm or give a different absolute path.
...
s.pluginConfigs['optimizer@plugin-rnd-lab'] = s.pluginConfigs['optimizer@plugin-rnd-lab'] || {};
s.pluginConfigs['optimizer@plugin-rnd-lab'].options = s.pluginConfigs['optimizer@plugin-rnd-lab'].options || {};
s.pluginConfigs['optimizer@plugin-rnd-lab'].options.experiments_root = process.argv[1];
...
(or `/reload-plugins`) may be needed before other optimizer skills pick up the new value — mention
it, don't assume. Next step: cd into the plugin's own repo and run `/optimizer:init` there — no
experiment name to type, optimizer figures out which plugin from cwd.
