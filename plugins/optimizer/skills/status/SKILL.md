...
`${user_config.experiments_root}`, tell the user to run `/optimizer:setup` first and stop.
...
`{"status":"fresh"}` → no experiment in this repo — tell the user `/optimizer:init` first, stop.
...
`/optimizer:init`'s create-env/create-mandate branches).
