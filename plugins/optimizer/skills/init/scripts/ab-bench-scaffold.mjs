#!/usr/bin/env node
/**
 * ab-bench-scaffold.mjs — mechanical folder/state.json plumbing for /optimizer:init.
 ...
 *     First-ever /optimizer:init in this repo. Creates .ab-bench/mandate-1/envs/env-1/,
 ...
 *     by /optimizer:understand, invoked mandatorily right after).
 *
 ...
 *     mandateFile (for /optimizer:understand to write) and envFile (for the skill to
...
function fail(msg) {
  console.error(`[optimizer-scaffold] ERROR: ${msg}`);
  process.exit(1);
}
...
function scaffoldTestenv(testenvDir, displayName, pluginRef) {
  ...
  // Seeded empty here so /optimizer:plan's ranking step has somewhere to read from on the
...
function cmdCreateEnv(repoRoot, experimentsRoot) {
  const abBenchDir = path.join(repoRoot, AB_BENCH_DIR);
  const state = loadState(abBenchDir);
  if (!state) fail('.ab-bench/state.json not found — run create-fresh first (/optimizer:init has never run in this repo)');
