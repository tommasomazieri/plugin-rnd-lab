---
description: >-
  Create (or extend) an optimizer experiment for the plugin whose repo you're CD'd into.
  Run this FROM the plugin-under-test's own repo — no path/name argument needed, optimizer
  ...
  /optimizer:understand to produce mandate.md. Re-running in a repo that already has
  .ab-bench/ offers: continue to /optimizer:plan, start a new env under the current mandate
  ...
  against control", "start an optimizer experiment", "I want to A/B test my plugin".
argument-hint: "[env-only|mandate] (optional — skip the menu and go straight to one branch)"
---

# optimizer: init experiment

Experiments root: `${user_config.experiments_root}`. If that's empty or still literally
reads `${user_config.experiments_root}`, tell the user to run `/optimizer:setup` first and
...
- **`{"status":"existing", ...}`** → this repo already has an experiment. Show the user a
  ...
  1. **Continue** — go straight to `/optimizer:plan` for the next run on the current env.
...
`prepareTimeoutSec`, default 900). optimizer runs it in each arm's workspace before the
...
is a plugin *and* something it depends on, declare both now: `/optimizer:plan` can then pin
...
Then **mandatorily** invoke `/optimizer:understand` (no argument needed — it reads
...
1. Mandatorily invoke `/optimizer:understand` to write the new `mandateFile` — full
...
- Every future Claude Code session started from this repo (or a subdirectory of it) will see
  ...
  need to re-state an experiment name to any optimizer skill.
- Next step: `/optimizer:plan` to define the task and DoDs for run-001 (or the next run).
