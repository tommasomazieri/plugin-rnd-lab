# .dod/ Integration Contract — optimizer ⇄ dod-lite (arm engine)

Status: **redesigned** (this session) — `plugins/dod-lite` in this repo is no longer a copy of the
standalone dod-lite plugin. It is a trimmed, hooks-only fork built specifically to be optimizer's
...
**Also redesigned (same session, layered on top of the above): the two-root split.** optimizer no
...
- **dod-lite (this repo's trimmed copy)** owns: check file format (`.dod/checks/<id>.<ext>`), the
  ...
  **It is an instrument, not a control loop.** Its Stop hook writes NOTHING to stdout: no
  ...
  Folding it into optimizer would enable optimizer's own skills inside the sessions being measured.
  It is registered in `marketplace.json` so it caches alongside optimizer — an installed plugin
...
- **optimizer** owns everything else: authoring REAL check files for a run (never placeholders),
...
<project-root>/.dod/                  ← resolved as path.join(cwd, '.dod') — a DIRECT CHILD of the
  ...
  config.json            WRITTEN AT SCAFFOLD TIME by optimizer — not optional here: {
                           "runners":          { ".ext": "command" },
...
`/optimizer:plan` budgets them deliberately and prefers an exit code wherever a criterion can be
...
stops being an A/B result and becomes a harness artifact — but it is recorded so `/optimizer:analyze`
...
what `/optimizer:analyze` expects, but it is purely cosmetic now — dod-lite ships no gate hook that
...
<plugin-under-test repo>/          ← where the MAIN session runs, and where .git lives
  .gitignore                       ← optimizer appends ".ab-bench/" here at first /optimizer:init
  .ab-bench/                       ← gitignored. Identity/config only — an arm NEVER reads this.
    ...
    mandate-N/
      mandate.md                   ← written by /optimizer:understand. ONE per mandate, shared by
      ...
      quality-rubric.md            ← written by /optimizer:understand. The versioned anchored
...
${user_config.experiments_root}/<plugin-folder-name>/mandate-N/env-M/    ← the TESTENV root,
  ...
  .dod/
    checks/                 ← REAL check files, authored by /optimizer:plan (script or prompt,
                               dod-lite's exact format). Shared + recycled across runs of this env.
    sessions/                ← owned entirely by optimizer's arm-session-start.mjs (sole writer of
...
`dod-checks.json` shape (written by `/optimizer:plan`, read by the `arm-session-start.mjs` hook):
...
The only remaining way to skip DoD tracking is per-run: if `/optimizer:plan` doesn't write
`runs/run-NNN/dod-checks.json` (or writes no entries for an arm), `arm-session-start.mjs` skips
registration silently for that arm and `/optimizer:analyze` leans on metrics + human verdict only.
...
the job undone at `/optimizer:analyze` (recorded per arm in `analysis/delivery.json`), or feeds
...
`<workspace>/.claude/settings.json` and fires optimizer's own SessionStart and Stop hooks. dod-lite
marks those subprocesses with `DOD_LITE_CHECKER=1` (its own recursion guard uses the same marker),
and both optimizer arm hooks bail on it: no manifest entry, no `.dod` registration, no turn count.
...
optimizer counts its own stop signals rather than trusting an operator's global counter hook: measured
...
"135 turns" against another arm's "1"). `/optimizer:fire` will not hand a run over until
...
## What `/optimizer:plan` must actually do
...
4. **Run `probe-checks.mjs` and pass it.** Every check is executed against a pristine clone of
   ...
   until the gate is green. `/optimizer:fire` re-runs the same probe, because a check edited
