...
Each one a thing a FUTURE run could test, in this exact shape so /optimizer:plan can rank it
...
## Harness defects
Problems with optimizer, dod-lite, the env config, or the checks themselves — separate from the
