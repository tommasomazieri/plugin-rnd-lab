...
/ab-bench:plan     rank open hypotheses against the declared priority pillar, pin each
...
/ab-bench:analyze  deterministic metrics + rubric-scored quality + session-comparator
...
/ab-bench:paper    regenerate lab/paper.md from the objective history, hypothesis
...
${user_config.experiments_root}/<plugin-folder-name>/mandate-N/env-M/   ← the TESTENV,
  ...
  lab/                the run-over-run layer (see "Continuity" below):
                         ...
                         paper.md          GENERATED by /ab-bench:paper — never hand-edited
  ...
  runs/run-NNN/
    ...
    baseline.json     schema 2: arms.{control,test}.pins — the resolved identity of every
                       declared artifact per arm, written by /ab-bench:plan's resolve-baseline.mjs
...
`/ab-bench:plan` pins each one per arm; `/ab-bench:fire` delivers them. An experiment with one
...
ab-bench runs it in each arm's workspace before the session starts, logs it under `.launch/`,
...
`/ab-bench:plan`'s checker-discovery step also looks inside each arm's pinned snapshot for the
...
- **The hypothesis ledger** (`lab/hypotheses.json`) is the unit of continuity. `/ab-bench:plan`
  ranks open hypotheses by `predicted_magnitude × confidence ÷ cost`, with anything that cannot
  move the declared priority pillar ranked below anything that can. `/ab-bench:analyze` resolves
...
the trajectory, and `/ab-bench:paper` says so where the fork happens.
...
| `skills/plan/scripts/resolve-baseline.mjs` | agent, in `/ab-bench:plan` | pin every declared artifact per arm to an immutable snapshot (ref worktree, HEAD sha, or content-hashed dirty-tree copy) and write `baseline.json` |
...
| `agents/session-comparator.md` | delegated by `/ab-bench:analyze` | LLM layer: per-pillar causal attribution over arm + subagent digests, evidence reading on the run's hypothesis, [SUBJECTIVE]-tagged score, and next steps emitted as rankable candidate hypotheses |
