...
be able to tell whether it held. Note it in the run folder so `/ab-bench:analyze` can
...
under `artifacts` (see `/ab-bench:init`), with its own repo and its own delivery mode. Most
...
`/ab-bench:fire` runs the same probe again and refuses to launch on a mismatch, so a check edited
