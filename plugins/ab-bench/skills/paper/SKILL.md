...
description: >-
  ...
  here", "write up the experiments", "state of the plugin", "/ab-bench:paper".
---

# ab-bench: write the development record
...
Run `ab-bench-scaffold.mjs detect` (see `/ab-bench:plan` step 0) to get `testenvRoot`. If
...
`/ab-bench:plan` uses to pick the next run. If the top-ranked open hypothesis is stale, or
...
`/ab-bench:analyze`, against the run that earned them, with the evidence in hand. Writing
